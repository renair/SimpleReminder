...
16. **Реєстрація за існуючими даними**

    `Очікування`: При спробі реєстрації за логіном що вже існує з'являлося б повідомлення що такий обліковий запис уже існує та процес реєстрації повертався б до стану редагування.

    `Робота програми`: При реєстрації перевірка на ідентичність логіну не проводиться.

    `Результат`: Потрібно реалізувати перевірку на ідентичність логіну під час реєстрації та сповіщати користувача про неможливість створення, якщо логін уже зареєстровано.

    `Виправлення`:

17. **Вихід з облікового запису без закриття програми**

    `Очікування`: В головному вікні зі списком нагадувань буде кнопка що дозволить вийти зі свого облікового запису не закриваючи програму.

    `Робота програми`: У вікні із загальним списком нагадуваннь у лівому нижньому кутку додана кнопка що дозволяє вийти зі свого облікового запису.

    `Результат`: Програма працює належним чином.

    `Виправлення`:

18. **Спрацювання збережених нагадувань після відкриття програми**

    `Очікування`: Після відкриття програми нагадування, встановлений час яких вже настав, будуть підсвічені червоним кольором.

    `Робота програми`: Після відкриття програми нагадування, встановлений час яких вже настав незалежно від того чи під час роботи програми чи в момент коли вона не працювала, підсвічуються відповідним чином.

    `Результат`: Нагадування працюють так, як очікувалось.

    `Виправлення`:

19. **Відображення лише своїх нагадувань**

    `Очікування`: Кожен користувач матиме змогу переглядати лише ті нагадування що він створив.

    `Робота програми`: Користувачу не відображаються нагадування створені іншими користувачами.
